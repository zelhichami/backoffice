...
// Fix local src/href URLs (non-external)
    ...
    public function generatePalette(Request $request): JsonResponse
    {
        ...
        $prompt_system = Setting::where('key', 'palette_prompt')->value('value');


        if (!$prompt_system) {
            return response()->json(['error' => 'No prompt provided and no global prompt is set.'], 400);
        }


        try {
            $imageUrl = $request->input('image_url');
            $prompt_user = $request->input('prompt_user');
...
EOT;
            $promptMessages = [
                [
                    'role' => 'system',
                    'content' => 'Respond ONLY with valid JSON in the exact structure below — do not include any explanations or extra formatting.'
                                 .$prompt_system
                ],
                [
                    'role' => 'user',
            ...
            if (json_last_error() !== JSON_ERROR_NONE) {
                ...
                return response()->json(['message' => json_encode($result->choices[0])], 500);
            }
